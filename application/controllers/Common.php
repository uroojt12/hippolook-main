...
class Common extends MY_Controller
{
	...
	function leave_review()
    {
        ...
        if ($this->input->post()) {
            ...
            $res['status'] = 0;
            $res['frm_reset'] = 0;
            $res['msg'] = showMsg('error', "You can't rate this product!");
            $res['redirect_url'] = 0;

            $encoded_id = $this->input->post('store');
            $id = intval(substr(doDecode($encoded_id), 3));

            if(!$row = $this->order_model->is_valid_mem_od($this->session->mem_id, $id)){
                $res['msg'] = showMsg('error', 'Something went worng!');
                exit(json_encode($res));
            }

            $this->form_validation->set_rules('rating', 'Rating', 'required|integer|greater_than_equal_to[1]|less_than_equal_to[5]', ['integer' => 'Please select rating']);
            ...
            if($this->form_validation->run() === FALSE) {
            	...
            	if ($post['rating'] > 5 || $post['rating'] < 0.1) {
            		$res['msg'] = showMsg('error', 'Please rate this product!');
            		exit(json_encode($res));
            	}

            	$save_data = array('from_id' => $this->session->mem_id, 'od_id' => $id, 'ref_id' => $row->p_id, 'ref_type' => 'product');
            	...
            	if(!$this->master->getRow('reviews', $save_data)){
            		...
            		$this->order_model->save_detail(['review' => 1], $id);
	...
	function favorite()
	{
		...
		$id = intval($id);
		if($row = $this->master->getRow('products', ['id' => $id]))
		{
