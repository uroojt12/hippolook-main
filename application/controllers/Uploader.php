...
class Uploader extends MY_Controller
{
	...
	{
		parent::__construct();
		// $this->isMemLogged($this->session->mem_type, true, false, array('owner', 'sitter'), false, false);
	...
	function save_profile_image()
	{
		...
		if ($_FILES['image']['name'] != "" && $this->input->post('image_type') != "") {
			...
			if (!empty($image['file_name'])) {
				...
				$res['p150'] = get_image_src($image['file_name'], 150, true);
				$res['p50'] = get_image_src($image['file_name'], 50, true);
				$res['upload_status'] = 1;
			} else {
				$res['upload_status'] = 0;
			}
	...
	function save_mul_images()
	{
		$res['upload_status'] = 0;
		// $token=doDecode($this->input->post('pk_key'));
		if (/*($this->master->getRow('members',array('mem_token'=>$token)) || $token=='admin') && */count($_FILES['images']['name'])>0 && $_FILES['images']['name'][0]!='' && in_array($this->input->post('type'), array('member', 'pet', 'booking'))) {
			...
			foreach ($_FILES['images']['name'] as $key => $file_name) {
				...
				if($_FILES['image_file']['name']!=''){
					...
					if (!empty($image['file_name'])) {
						$this->master->save('gallery', array('image' => $image['file_name'], 'ref_type' => $this->input->post('type'), 'date' => date('Y-m-d H:i:s')));
						$res['image_path'][] = get_image_src($image['file_name'],300,true);
						$res['image'][] = $image['file_name'];
					}
				}
			}
