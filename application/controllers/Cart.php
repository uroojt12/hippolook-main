...
class Cart extends My_Controller
{
    ...
    public function information()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() === FALSE) {
                ...
                $delivery_cost = $this->data['gtotal'] > $this->data['site_settings']->site_free_shipping ? 0 : $country_row->delivery_cost;
                // exit($this->data['gtotal'] .' = '. $this->data['site_settings']->site_free_shipping.' = '.$delivery_cost);
    ...
    public function payment()
    {
        ...
        if ($this->input->post()) {
            ...
            /*foreach ($cart_items as $key => $item) {
                if (!$product_row = $this->product_model->is_valid_product($item->p_id, $item->qty)) {
                    $res['msg'] = showMsg('error', "{$item->title} is out of Stock!");
                    exit(json_encode($res));
                }
            }*/

            $cart_group_items = $this->cart_model->get_cart_group_products();
            foreach ($cart_group_items as $key => $group_item) {
                if (!$product_row = $this->product_model->is_valid_product($group_item->p_id, $group_item->qty)) {
                    $res['msg'] = showMsg('error', "'{$group_item->title}' doesn't have enough Stock!");
                    exit(json_encode($res));
                }
            }
    ...
    {
        $id = intval($id);
        if ($product = $this->product_model->is_valid_product($id, 1)) {
            if($this->input->post()) {
                $res = array();
                $res['status'] = 0;
                $res['frm_reset'] = 0;
                ...
                if ($product->stock <= $this->cart_model->item_count($id)) {
                    // setMsg('error', "Product doesn't have enought stock!");
                    // redirect('cart', 'refresh');
                    /*redirect('product-detail/'.$id.'/'.toSlugUrl($product->title), 'refresh');
                    exit;*/
                    $res['msg'] = "<p>Product doesn't have enought stock!</p>";
                    exit(json_encode($res));
                }
                ...
                if (!empty($product->sunglasses))
                    ...
                    $this->form_validation->set_rules('glasses', 'Choose Glasses', 'required|in_list[6,2,3,4,5]', array("in_list" => "Please select valid {field} option"));
                else
                    $this->form_validation->set_rules('glasses', 'Choose Glasses', 'required|in_list[1,2,3,4,5]', array("in_list" => "Please select valid {field} option"));
                ...
                switch ($post['glasses']) {
                    case 1:
                        ...
                        $this->form_validation->set_rules('frame_lens_type', 'Lens Type', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        break;
                    case 2:
                        ...
                        $this->form_validation->set_rules('prescription_lens_type', 'Lens Type', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        ...
                        $this->form_validation->set_rules('prescription_classic_lenses', 'Classic Lenses', 'required|in_list[first,second,third]', array("in_list" => "Please select valid {field} option"));
                        break;
                    case 3:
                        ...
                        $this->form_validation->set_rules('polarized_lens_type', 'Lens Type', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        
                        if ($post['polarized_lens_type'] == 'second') {
                        ...
                        $this->form_validation->set_rules('polarized_classic_lenses', 'Classic Lenses', 'required|in_list[first,second,third]', array("in_list" => "Please select valid {field} option"));
                    ...
                    case 4:
                        ...
                        $this->form_validation->set_rules('transition_lens_type', 'Lens Type', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        
                        if ($post['transition_lens_type'] == 'second') {
                            // $this->form_validation->set_rules('dates', 'Dates', 'required', array('required' => 'Please select {field}!'));
                        }
                        ...
                        $this->form_validation->set_rules('transition_lens_property', 'Lens Property', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        ...
                        $this->form_validation->set_rules('transition_classic_lenses', 'Classic Lenses', 'required|in_list[first,second,third]', array("in_list" => "Please select valid {field} option"));
                        break;
                    case 5:
                        ...
                        $this->form_validation->set_rules('progressive_lens_type', 'Lens Type', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        $this->form_validation->set_rules('progressive_lens_property', 'Lens Property', 'required|in_list[first,second]', array("in_list" => "Please select valid {field} option"));
                        ...
                        $this->form_validation->set_rules('progressive_classic_lenses', 'Classic Lenses', 'required|in_list[first,second,third]', array("in_list" => "Please select valid {field} option"));
                        break;
                }
                ...
                $item = array(
                    ...
                    'glasses' => $glasses[$post['glasses']-1]->title
                );
                $content = unserialize($glasses[$post['glasses']-1]->detail);
                ...
                switch ($post['glasses']) {
                    case 1:
                        ...
                        $item['lens_type'] = $content["type_".$post['frame_lens_type']."_title"];
                        $item['lens_type_price'] = $glasses[0]->{"type_".$post['frame_lens_type']."_price"};
                        break;
                    case 2:
                        ...
                        $item['lens_type'] = $content["type_".$post['prescription_lens_type']."_title"];
                        $item['lens_type_price'] = $glasses[1]->{"type_".$post['prescription_lens_type']."_price"};

                        $item['classic_lenses'] = $content["classic_".$post['prescription_classic_lenses']."_title"];
                        $item['classic_lenses_price'] = $glasses[1]->{"classic_".$post['prescription_classic_lenses']."_price"};
                    ...
                    case 3:
                        ...
                        $item['lens_type'] = $content["type_".$post['polarized_lens_type']."_title"];
                        $item['lens_type_price'] = $glasses[2]->{"type_".$post['polarized_lens_type']."_price"};
                        if ($post['polarized_lens_type'] == 'second') {
                            $item['logic_lens_type'] = 'second';
                        ...
                        $item['classic_lenses'] = $content["classic_".$post['polarized_classic_lenses']."_title"];
                        $item['classic_lenses_price'] = $glasses[2]->{"classic_".$post['polarized_classic_lenses']."_price"};
                    ...
                    case 4:
                        ...
                        $item['lens_type'] = $content["type_".$post['transition_lens_type']."_title"];
                        $item['lens_type_price'] = $glasses[3]->{"type_".$post['transition_lens_type']."_price"};
                        ...
                        $item['lens_property'] = $content["property_".$post['transition_lens_property']."_title"];
                        $item['lens_property_price'] = $glasses[3]->{"property_".$post['transition_lens_property']."_price"};

                        $item['classic_lenses'] = $content["classic_".$post['transition_classic_lenses']."_title"];
                        $item['classic_lenses_price'] = $glasses[3]->{"classic_".$post['transition_classic_lenses']."_price"};
                    ...
                    case 5:
                        ...
                        $item['lens_type'] = $content["type_".$post['progressive_lens_type']."_title"];
                        $item['lens_type_price'] = $glasses[4]->{"type_".$post['progressive_lens_type']."_price"};

                        $item['lens_property'] = $content["property_".$post['progressive_lens_property']."_title"];
                        $item['lens_property_price'] = $glasses[4]->{"property_".$post['progressive_lens_property']."_price"};
                        
                        $item['classic_lenses'] = $content["classic_".$post['progressive_classic_lenses']."_title"];
                        $item['classic_lenses_price'] = $glasses[4]->{"classic_".$post['progressive_classic_lenses']."_price"};
                ...
                redirect('cart', 'refresh');
                exit;*/
                $res['redirect_url'] = site_url('cart');
                $res['msg'] = "<p>Product added to cart successfully!</p>";
                $res['status'] = 1;
                exit(json_encode($res));
            }
        } else {
